# Plan Integrity

## Baseline-Safe Commands

These commands are valid before Stage 5 code changes because Stage 4 already created the relevant files in this working tree:

- `python tools/train_voice_candidates.py --help`
- `python -m unittest discover -s tests`
- `bash scripts/run_train_voice_candidates_smoke.sh`
- `python -m compileall -q tools scripts external/Qwen3-TTS/finetuning external/Qwen3-TTS/qwen_tts`
- `bash -n scripts/run_prepare_data.sh scripts/run_sft_0_6b.sh scripts/run_voice_filter_smoke.sh scripts/run_train_voice_candidates_smoke.sh`
- `git diff --check`
- `git status --short --ignored`

## Requires Phase Work

- Assertions for `checkpoint_gate` rows require Phase 2.
- Assertions for `candidate_selection` rows and `candidate_manifest.json` require Phase 3.
- Docs grep for new hard reject contract requires Phase 4.

## External / Env

- Real GPU training.
- Real `--text_match_backend faster-whisper` transcription.
- Real speaker embedding similarity.

