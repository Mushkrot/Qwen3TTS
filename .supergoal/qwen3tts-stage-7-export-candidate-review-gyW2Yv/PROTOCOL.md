...
All paths below are rooted at this run's artifact directory (`.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv`) — the concrete namespaced path is baked in when this file is copied at Stage 7, so two runs in the same working tree read and write entirely separate artifacts.

## The loop

Repeat until `SUPERGOAL_RUN_COMPLETE` is printed:

1. Read `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/STATE.md`. Find `Current phase: N`.
2. Read `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/phases/phase-N.md`. This is your full work spec.
...
2. Re-read `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/ROADMAP.md` and pull every phase's acceptance criteria fresh from the original plan.
...
5b. **Deliverable check** — for each phase block in `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/ROADMAP.md`, parse the `**Deliverables:**` bullets. For every bullet that names a file path or glob:
   - Read `Baseline ref:` from `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/STATE.md`.
   - Run `bash .supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/repo-state.sh deliverable <baseline-ref> "<path>"`. It compares the **complete working tree** (committed + staged + unstaged + deleted) against the baseline and detects untracked new files separately, printing `present — <evidence>` (exit 0) or `missing` (exit 1). An invalid/unavailable baseline degrades to a filesystem existence check. Strategy: `references/repo-state-comparison.md`.
...
2. Write `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/phases/audit-fix-<round>.md` — a focused fix spec that targets only the failing criteria. Forbid scope creep. Use the affected phases' original VERIFY as the success gate.
...
2. Append the probe to `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/STATE.md` failure log.
...
2. Write a focused **fix spec** at `.supergoal/qwen3tts-stage-7-export-candidate-review-gyW2Yv/phases/phase-N.fix.md`. The fix spec:
